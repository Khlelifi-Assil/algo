...
class SVMRecommendationModel:
    ...
    def load_data(self):
        ...
        self.X = data_df['Name']  # Adapt to your text data
        self.y = data_df['Rate']

    def train_model(self):
        X_train, X_test, y_train, y_test = train_test_split(self.X, self.y, test_size=0.2, random_state=42)

        tfidf_vectorizer = TfidfVectorizer()  # Convert text to lowercase
    ...
    def evaluate_model(self):
        ...
        X_test_tfidf = tfidf_vectorizer.transform(self.X)  # Use the same vectorizer
        y_pred = svm_model.predict(X_test_tfidf)

        report = classification_report(self.y, y_pred, target_names=['1', '2', '3', '4', '5'])
        print(report)
        with open("svm_resultats.txt", "w") as f:
            f.write(report)
...
if __name__ == "__main__":
    model = SVMRecommendationModel('input/dataset/customers.json',
